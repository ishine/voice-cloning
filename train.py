...
class FilterModule(logging.Filter):
    ...
    def filter(self, rec):
        return not rec.name.startswith(self.modulename) and not rec.name.startswith(f"DEBUG:{self.modulename}")
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  net_g.train()
  net_d.train()
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, embed_ref) in enumerate(train_loader):
...
def evaluate(hps, generator, eval_loader, writer_eval):
    generator.eval()
    with torch.no_grad():
      for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, embed_ref) in enumerate(eval_loader):
        ...
        break
      y_hat, attn, mask, *_ = generator.module.infer(x, x_lengths, embed_ref, max_len=1000)
