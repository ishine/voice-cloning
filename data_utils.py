...
class TextAudioLoader(torch.utils.data.Dataset):
    ...
    def gen_audio_map(self, audiopaths_and_text):
        audio_map = {}
        for line in audiopaths_and_text:
            idx = line[1]
            if idx not in audio_map:
                audio_map[idx] = []
            audio_map[idx].append(line[0])
        return audio_map
    ...
    def get_audio_text_pair(self, audiopath_and_text):
        ...
        audiopath, au_id, text = audiopath_and_text[0], audiopath_and_text[1], audiopath_and_text[2]
...
class TextAudioCollate():
    ...
    def __call__(self, batch):
        ...
        max_ref_len = max([len(x[3]) for x in batch])
        ...
        if self.return_ids:
            return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, ref_padded, ids_sorted_decreasing

        return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, ref_padded
