...
class TextEncoder(nn.Module):
  def __init__(self,
    ...
    self.encoder = attentions.Encoder(
      ...
      kernel_size,
      p_dropout)
    # self.ref_embed = nn.Conv1d(256, hidden_channels, 1)
    # self.ref_concat = nn.Conv1d(hidden_channels + 256, hidden_channels, 1)
  ...
  def forward(self, x, x_lengths):
    ...
    # if embed_ref.shape[0] != x.shape[0]:
    #     embed_ref = embed_ref[:len(x)].cuda(0, non_blocking=True)
    # embed_ref = torch.unsqueeze(embed_ref, axis=-1)
    # embed_ref = torch.broadcast_to(embed_ref, (x.shape[0], 256, x.shape[-1]))
    # # embed_ref = self.ref_embed(embed_ref)
    # x = torch.cat((x, embed_ref), dim=1)
    # x = self.ref_concat(x)
    # del embed_ref
